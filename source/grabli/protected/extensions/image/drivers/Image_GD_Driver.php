...
class Image_GD_Driver extends Image_Driver {
	...
	*/
	public function watermark($properties) {
		...
		// считаем новую высоту водяного знака
		$newWatermarkHeight = intVal(($imWidth / $watermarkWidth) * $watermarkHeight);
		
		// ситаем положение сверху
		$watermarkTop = intVal($imHeight - $newWatermarkHeight);
		
		imageAlphaBlending($img, TRUE);
		// imagecopy($image,$watermarkSrc, 0, 0, 0,0, $watermarkSrcWidth, $watermarkSrcHeight);
		// imagecopyresampled ($this->_image, $watermarkSrc, 0,                   $watermarkTop,      0, 0, $imageWidth,                 $newWatermarkHeight, $watermarkSrcWidth, $watermarkSrcHeight);
		imagecopyresampled ($img, $watermark,  0, $watermarkTop, 0, 0, $imWidth, $newWatermarkHeight, $watermarkWidth, $watermarkHeight);
